...
class SamplingModelHomogeneous (ISamplingModel):
    ...
    def sampleData(self, nSamples, ProbLabelFlip, std):
        ...
        for key in self._projectedData:
            ...
            xCandidate = np.random.uniform(min(d), max(d), nSamples)[:, np.newaxis]
...
#        
    def sampleAndPlotData(self, targetAxis, nSamples, ProbLabelFlip, sigmas):#remove sigmas
...
#            for ii, sigma in enumerate(sigmas):
            selectedSamples, flipLabel = self.sampleData(std = 1, nSamples=nSamples, 
            ...
            yi = (0.05 * (yi / max(yi))) - 0.25 + classIdx*0.05
